...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::410692821451:role/iep-provisioner-gha-exec"
  }
  ...
  default_tags {
    tags = {
      ...
      cost-key         = "1386398"
      github-repo      = "rosa-infra"
      organization     = "ccoe"
      runtime-env      = "rosa-prod"
      workload         = "cap"
    }
  }
}
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/rosa-infra/prod/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
