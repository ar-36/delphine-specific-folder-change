...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::035426313441:role/iep-provisioner-gha-exec"
  }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/rosa-infra/dev/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
