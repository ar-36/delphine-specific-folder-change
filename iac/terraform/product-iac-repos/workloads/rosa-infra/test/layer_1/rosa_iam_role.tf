## IAM ROLE FOR ROSA
data "aws_iam_policy_document" "rosa-infra_test_instance_assume_gha_role_policy" {
  ...
  statement {
    ...
    principals {
      type        = "Federated"
      identifiers = ["arn:aws:iam::276378825251:oidc-provider/token.actions.githubusercontent.com"]
    }
    ...
    condition {
      ...
      values = [
        "repo:CPC-SCP/rosa-infra:job_workflow_ref:CPC-SCP/rosa-infra/.github/workflows/rosa_test_deploy.yml@refs/heads/*",
        "repo:CPC-SCP/rosa-infra:job_workflow_ref:CPC-SCP/rosa-infra/.github/workflows/rosa_test_configure.yml@refs/heads/*"
      ]
    }
  }
}

resource "aws_iam_role" "rosa-infra_test_workload_provisioner_role" {
  # checkov:skip=CKV_AWS_274:Allow AdministratorAccess usage here because it is appropriate for usage by TF + GHA

  assume_role_policy = data.aws_iam_policy_document.rosa-infra_test_instance_assume_gha_role_policy.json
