...
data "aws_iam_policy_document" "gha" {
  statement {
    ...
    principals {
      identifiers = ["arn:aws:iam::211125409189:oidc-provider/token.actions.githubusercontent.com"]
      type        = "Federated"
    }
...
resource "aws_iam_role" "gha" {
  ...
  name                 = "iep-edapa-test-infra-test-gha-exec"
  path                 = "/"
  ...
  # permissions_boundary = aws_iam_policy.edapa-test-infra_test_role_permission_boundary.arn
