...
data "aws_iam_policy_document" "gha" {
  statement {
    ...
    principals {
      identifiers = ["arn:aws:iam::730335505732:oidc-provider/token.actions.githubusercontent.com"]
      type        = "Federated"
    }
...
resource "aws_iam_role" "gha" {
  ...
  name                 = "iep-edapa-test-infra-prod-gha-exec"
  path                 = "/"
  ...
  # permissions_boundary = aws_iam_policy.edapa-test-infra_prod_role_permission_boundary.arn
