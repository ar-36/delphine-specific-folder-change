...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::220813086125:role/iep-provisioner-gha-exec"
  }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/edapa-test-infra/dev/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
