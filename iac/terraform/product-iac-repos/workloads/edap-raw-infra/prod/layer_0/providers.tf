...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::488152613919:role/iep-provisioner-gha-exec"
  }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/edap-raw-infra/prod/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
