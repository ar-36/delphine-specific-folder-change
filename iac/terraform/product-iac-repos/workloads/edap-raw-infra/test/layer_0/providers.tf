...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::606096854338:role/iep-provisioner-gha-exec"
  }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/edap-raw-infra/test/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
