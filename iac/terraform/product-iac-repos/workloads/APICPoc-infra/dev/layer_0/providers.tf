...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::533267330468:role/iep-provisioner-gha-exec"
  }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/APICPoc-infra/dev/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
