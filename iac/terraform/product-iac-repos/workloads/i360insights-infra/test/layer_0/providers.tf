...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::527351886011:role/iep-provisioner-gha-exec"
  }
  ...
  default_tags {
    tags = {
      ...
      cost-key         = "1386398"
      github-repo      = "i360insights-infra"
      organization     = "ccoe"
      runtime-env      = "test"
      workload         = "i360insights"
    }
  }
}
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/i360insights-infra/test/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
