...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::744659032260:role/iep-provisioner-gha-exec"
  }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/i360insights-infra/prod/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
