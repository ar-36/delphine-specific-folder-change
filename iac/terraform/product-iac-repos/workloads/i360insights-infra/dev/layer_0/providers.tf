...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::004802276251:role/iep-provisioner-gha-exec"
  }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/i360insights-infra/dev/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
