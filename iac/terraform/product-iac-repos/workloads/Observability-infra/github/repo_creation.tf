...
resource "github_actions_repository_oidc_subject_claim_customization_template" "main" {
  ...
  provider           = github
  repository         = "Observability-infra"
  use_default        = false
}
...
module "github_repo_teams" {
  ...
  product_name             = "Observability-infra"
  repo_name                = "Observability-infra"
}
