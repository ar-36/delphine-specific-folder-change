...
data "aws_iam_policy_document" "gha" {
  statement {
    ...
    principals {
      identifiers = ["arn:aws:iam::654654332377:oidc-provider/token.actions.githubusercontent.com"]
      type        = "Federated"
    }
...
resource "aws_iam_role" "gha" {
  ...
  name                 = "iep-Observability-infra-test-gha-exec"
  path                 = "/"
  ...
  # permissions_boundary = aws_iam_policy.Observability-infra_test_role_permission_boundary.arn
