...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::211125654669:role/iep-provisioner-gha-exec"
  }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/CAPTest-infra/test/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
