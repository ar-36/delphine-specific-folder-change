...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::851725380142:role/iep-provisioner-gha-exec"
  }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/CAPTest-infra/dev/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
