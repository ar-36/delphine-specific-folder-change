...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::730335578993:role/iep-provisioner-gha-exec"
  }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/CAPTest-infra/prod/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
