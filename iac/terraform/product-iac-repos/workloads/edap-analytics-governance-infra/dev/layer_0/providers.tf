...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::070318300770:role/iep-provisioner-gha-exec"
  }
  ...
  default_tags {
    tags = {
      ...
      cost-key         = "1386398"
      github-repo      = "edap-analytics-governance-infra"
      organization     = "ccoe"
      runtime-env      = "dev"
      workload         = "edapa-gov"
    }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/edap-analytics-governance-infra/dev/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
