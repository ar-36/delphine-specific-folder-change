...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::416446934920:role/iep-provisioner-gha-exec"
  }
  ...
  default_tags {
    tags = {
      ...
      cost-key         = "1386398"
      github-repo      = "edap-governance-infra"
      organization     = "ccoe"
      runtime-env      = "prod"
      workload         = "edap-gov"
    }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/edap-governance-infra/prod/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
