...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::884561156600:role/iep-provisioner-gha-exec"
  }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/iep-test-workload-1-infra/test/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
