...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::482699811241:role/iep-provisioner-gha-exec"
  }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/iep-test-workload-1-infra/dev/kms/terraform.tfstate"
    region         = "ca-central-1"
  }
}
