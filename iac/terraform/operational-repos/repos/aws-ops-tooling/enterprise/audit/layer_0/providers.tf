...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::022779729608:role/iep-provisioner-gha-exec"
  }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/aws-ops-tooling/audit/layer_0/terraform.tfstate"
    region         = "ca-central-1"
  }
}
