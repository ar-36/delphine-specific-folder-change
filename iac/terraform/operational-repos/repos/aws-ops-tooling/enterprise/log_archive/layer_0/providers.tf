...
provider "aws" {
  ...
  assume_role {
    role_arn = "arn:aws:iam::110802634245:role/iep-provisioner-gha-exec"
  }
...
terraform {
  ...
  backend "s3" {
    ...
    key            = "iep-provisioner/product-iac-repos/workloads/aws-ops-tooling/log_archive/layer_0/terraform.tfstate"
    region         = "ca-central-1"
  }
}
